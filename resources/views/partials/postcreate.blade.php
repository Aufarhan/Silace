<form id="laporanForm" method="post" action="{{ route('posts.store') }}" enctype="multipart/form-data" class="container">
  ...
  <div class="flex flex-col gap-y-[20px]">
      <div>
          <label for="title" class="block mb-2">
              <h2 class="text-primary dark:text-primary" data-title="1. Mengisi Laporan" data-intro="Kamu akan mulai dengan mengisi laporan, mulai dari judul laporan, jenis laporan hingga lokasi kejadian." data-step="1">Judul Laporan</h2>
          </label>
              ...
              <label for="images" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 hover:border-primary border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-primary dark:hover:bg-gray-600">
                  <div class="flex flex-col items-center justify-center pt-5 pb-6">
                      ...
                      <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                          <span class="font-semibold">Tekan disini untuk mengunggah lampiran</span> atau drag dan drop
                      </p>
      ...
      <div>
          ...
          @error('body')
          <p class="peer-invalid:visible text-red-700">{{ $message }}</p>
          @enderror
      </div>
  </div>
</div>
      ...
      <div class="flex flex-col gap-y-[20px]">
          ...
          <div>
              <label for="lokasi" class="block mb-2">
                ...
                <p class="">Semakin lengkap semakin sigap!</p>
              </label>
    ...
    <div class="step gapy">
      <div>
        <div class="p-2">
          ...
          <p class="">Cek kembali laporanmu sebelum mengirim laporan</p>
        </div>
      ...
      <div>
        <div class="gapy">
          <label for="status_id"  class="block mb-4">
            ...
            <p class="">Bagaimana laporanmu ingin dilihat?</p>
          </label>
...
<script>
  function showText() {
    ...
    // Menampilkan nilai di elemen laintrus
...
<script>
  document.getElementById('images').addEventListener('change', function(event) {
      ...
      var updatedFiles = new DataTransfer(); // DataTransfer untuk memperbarui input file
          ...
          (function(file) {
              ...
              reader.onload = function(e) {
                  ...
                  removeButton.onclick = function() {
                      ...
                      var filesArray = Array.from(updatedFiles.files);
                      updatedFiles.items.clear(); // Kosongkan DataTransfer
                      filesArray.forEach(f => {
                          if (f !== file) {
                              updatedFiles.items.add(f); // Tambahkan file yang tidak dihapus
                          }
                      });
                      document.getElementById('images').files = updatedFiles.files; // Perbarui input file
                  };

                  imageContainer.appendChild(image);
                  imageContainer.appendChild(removeButton);
                  previewContainer.appendChild(imageContainer); // Tampilkan pratinjau gambar di dalam div
              }
              
              reader.readAsDataURL(file);
              updatedFiles.items.add(file); // Tambahkan file ke DataTransfer
          })(files[i]);
      }

      // Perbarui input file
      document.getElementById('images').files = updatedFiles.files;
  });
</script>
